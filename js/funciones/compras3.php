    ...
    if($num_rows > 0){ ?>
        ...
        <?php
            while($row = _fetch_array($query)){
                ...
                $id_producto_bp= $row['id_producto_bp'];
                $id_posicion = $row['id_posicion'];

                $sql_contenedor="SELECT descripcion FROM producto_base where id_producto_base='$id_punta'";
                $query2 = _query($sql_contenedor);
                $row2 = _fetch_array($query2 );
                $descripcion_contenedor=strtoupper($row2["descripcion"]);
                if(  $id_producto_bp>0 || $id_punta>0){
                    $descripcion.="<span class='text-warning'>&nbsp;(DENTRO DE &nbsp;".$descripcion_contenedor.")</span>";
                }

                $fotos=tieneimagen($id_producto,$imagen);
                $ubicacion=ubicacion($id_posicion);

                $btnVer="<a  class='btn btn-success' data-toggle='modal' href='ver_partes.php?id_producto=".$id_producto."' data-target='#viewModal' data-refresh='true'><i class='fa fa-eye'></i> </a>";
                $btnSelect='<button type="button" id="btnSelect" class="btn btn-primary"><i class="fa fa-check"></i> </button>';
                $nuevo_value='<input id="nuevo" name="nuevo" type="hidden" value="'.$nuevo.'">';
                $stock_value='<input id="stock" name="stock" type="hidden" value="'.$stock.'">';
          ...
          if ($stock>0 && $precio>0){
            ...
            <td><?php echo $stock_value;?><h5><?php echo $descripcion;?></h5></td>
            <td><h5 class='text-success'><?php echo $marca;?></h5></td>
            <td><h5 class='text-success'><?php echo $modelo;?></h5></td>
            <td><h5 class='text-success'><?php echo $anio;?></h5></td>
            <td><?php echo $nuevo_value;?><h5 class='text-warning'><?php echo $precio; ?></h5></td>
            <td><h5 class='text-success'><?php echo $stock;?></h5></td>
            <td><h5 class='text-primary'><?php echo $ubicacion;?></h5></td>
            <td><h5 class='text-success'><?php echo $fotos;?></h5></td>
            <td><h5 class='text-success'><?php echo $btnVer.' '.$btnSelect; ?></h5></td>
					</tr>
          <?php
            }
          }
