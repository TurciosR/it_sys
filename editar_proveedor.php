...
function initial() 
{
    ...
    $sql = _query("SELECT * FROM proveedores WHERE id_proveedor='$id_proveedor'");
...
function insertar()
{
    ...
    $sql_exis=_query("SELECT id_proveedor FROM proveedores WHERE nit ='$nit' AND id_proveedor != '$id_proveedor'");
    $num_exis = _num_rows($sql_exis);
    if($num_exis > 0)
    {
        $xdatos['typeinfo']='Error';
        $xdatos['msg']='Este Proveedor ya fue registrado!';
    }
    else
    {
        $table = 'proveedores';
...
function cxpp()
{
    ...
    $id_proveedor = $_POST["id_proveedor"];
    $table = "";
    $sql = _query("SELECT * FROM cxp WHERE id_proveedor='$id_proveedor' AND saldo_pend>0 AND CAST(fecha AS DATE) BETWEEN '$fini' AND '$fin' ORDER BY CAST(fecha as DATE) DESC");
    if(_num_rows($sql)>0)
    {
        $entrada = 0;
        $salida = 0;
