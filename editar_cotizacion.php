...
function initial()
{
    ...
    $sql_cotizacion = _query("SELECT * FROM cotizacion WHERE id_cotizacion = '$id_cotizacion'");
    if(_num_rows($sql_cotizacion) > 0)
    {
      $row_cotizacion = _fetch_array($sql_cotizacion);
      $id_cliente = $row_cotizacion["id_cliente"];
      $sql_clien = _query("SELECT * FROM clientes WHERE id_cliente = '$id_cliente'");
      $name = "";
      if(_num_rows($sql_clien) > 0)
      {
        $row_clie = _fetch_array($sql_clien);
        $nombre = $row_clie["nombre"];
        $name = $nombre;
      }
      $fecha_coti = $row_cotizacion["fecha"];
      $vigencia = $row_cotizacion["vigencia"];
      $total = $row_cotizacion["total"];
      $id_vendedor = $row_cotizacion["id_vendedor"];
      $tipo_cotizacion = $row_cotizacion["tipo_cotizacion"];
      ...
      $atencion = $row_cotizacion["atencion"];
      $gestion = $row_cotizacion["gestion"];
      $ofrecemos = $row_cotizacion["ofrecemos"];
      $garantia = $row_cotizacion["garantia"];
      $entrega = $row_cotizacion["entrega"];
      $dias_credito = $row_cotizacion["dias_credito"];
      $pago = $row_cotizacion["pago"];
      $tipo_doc = $row_cotizacion["tipo_doc"];
      $pago = $row_cotizacion["pago"];
      $dias_credito = $row_cotizacion["dias_credito"];
      if($pago == "CRE")
      {
                      ...
                      <div id='form_factura_compra'>
                          ...
                          <div class="col-md-12">
                            ...
                            <div  class="form-group col-md-2">
                                ...
                                <select name='tipo_doc' id='tipo_doc' class='form-control select2'>
                                  <option value='COF'
                                  <?php if($tipo_doc == "COF"){echo "selected";} ?>
                                  >FACTURA</option>
                                  <option value='CCF'
                                  <?php if($tipo_doc == "CCF"){echo "selected";} ?>
                            ...
                            <div class='col-md-2'>
                              <div class='form-group'>
                                <label>Fecha:</label>
                                <input type='text' placeholder='Fecha' class='datepick2 form-control' value='<?php echo ED($fecha_coti); ?>' id='fecha' name='fecha'></div>
                            </div>
                            <div class="form-group col-md-2">
                                <label>Forma de pago</label>
                                <select name='con_pago' id='con_pago' class='form-control'>
                                  <option value='CON'
                                  <?php if($pago == "CON"){ echo "selected"; } ?>
                                  >Contado</option>
                                  <option value='CRE'
                                  <?php if($pago == "CRE"){ echo "selected"; } ?>
                                  >Credito</option>
                                  <option value='CHE'
                                  <?php if($pago == "CHE"){ echo "selected"; } ?>
                                  >Cheque</option>
                                </select>
                            </div>
														<div class="col-md-1">
                            ...
                            <div class="col-md-1">
			                        <div class="form-group has-info">
			                          ...
			                          <input type="text"  class='form-control' id="dias_entrega" value="<?php echo $entrega; ?>">
			                        </div>
			                      </div>
                        ...
                        <div class='row'>
                          ...
                          <div class="col-md-12">
                            ...
                            <div class="col-md-1">
                              <div class='form-group' id="caja_dias" <?php echo $text_caja; ?>>
                                <label>Días Crédito</label>
                                <input type='text' class='form-control' id='dias_credito' name='dias_credito' value="<?php echo $dias_credito; ?>">
                              </div>
                            </div>
                            <div class="col-md-7">
                  ...
                  <div class="ibox-content">
                    ...
                    <section>
                      ...
                      <div class="col-md-12">
                          ...
                          <div class="table100 ver1 m-b-10">
                            ...
                            <div class="table100-body js-pscroll">
                              <table id="inventable">
                                <tbody >
                                  <?php
                                  $sql_detalles = _query("SELECT * FROM cotizacion_detalle WHERE id_cotizacion = '$id_cotizacion'");
                                  if(_num_rows($sql_detalles) > 0)
                                  {
                                    $lista = "";
                                    $filas = 1;
                                    while ($row_detalles = _fetch_array($sql_detalles))
                                    {
                                      ...
                                      if($tipo_prod_serv == "PRODUCTO")
                                      {
                                        ...
                                        if ($nrow1>0)
                                        {
                                          ...
                                          while ($row=_fetch_array($sql_p))
                                          {
                                            ...
                                            else
                                            {
                                              ...
                                              $preciof=$row['total_iva'];
                                            }

                                            $select.="<option value='".$preciof."'";
                                        ...
                                        if($perecedero == 1)
                                        {
                                          $estado = "<td class='cell100 column10 text-success'><div class='col-xs-2'><input type='text'  class='form-control vence' id='vence' name='vence'  value='' style='width:100px;'></div></td>";
                                        }
                                        else
                                        {
                                          $estado = "<td class='cell100 column10 text-success'><div class='col-xs-2'><input type='hidden'  class='form-control vence' id='vence' name='vence'  value='' style='width:100px;'></div></td>";
                                        }
                                      }
                                      ...
                                      $lista .= '<tr class="row100 head" id=' . $filas . '>';
                                      $lista .= "<td class='cell100 column10 text-success'><input type='hidden'  id='bandera' name='bandera' value='".$bandera."'><input type='hidden'  id='id_producto' name='id_producto' value='" . $id_pro . "'>" . $id_pro . "</td>";
                                      $lista .= '<td  class="cell100 column55 text-success"><input type="hidden"  class="form-control descripcion_ser" id="descripcion_ser" name="descripcion_ser" value="">' . $descripcion . '</td>';
                                      $lista .= "<td class='cell100 column10 text-success'>".$select."</td>";
                                      //tr_add += "<td class='cell100 column10 text-success'><div class='col-xs-2'>"+precios+"</div></td>";
                                      $lista .= "<td class='cell100 column10 text-success'><div class='col-xs-2'><input type='text'  class='form-control decimal' id='cant' name='cant' value='".$cantidad."' style='width:60px;'></div></td>";
                                      $lista .= $estado;
                                      $lista .= '<td class="Delete cell100 column5 text-success"><input  id="delprod" type="button" class="btn btn-danger fa pull-right"  value="&#xf1f8;"></td>';
                                      $lista .= '</tr>';
                        ...
                        <input type="hidden" name="id_cotizacion" id="id_cotizacion" value="<?php echo $id_cotizacion; ?>"><br>
                    <div>
...
function editar()
{
  ...
  if ($cuantos>0)
  {
    ...
    if ($nrows_fact==0)
    {
      ...
      $w = "id_cotizacion='".$id_cotizacion."'";
      $update_co = _update($table_fact, $form_data_fact, $w);
      //$id_fact= _insert_id();
    }
    $table_fact_det= 'cotizacion_detalle';
    $wd = "id_cotizacion='".$id_cotizacion."'";
    $delete_co = _delete($table_fact_det,$wd);
    if($delete_co)
    {
      ...
      foreach ($array as $fila)
      {
        if ($fila['precio']>=0 && $fila['cantidad']>0)
        {
          ...
          $precio_venta=str_replace(",", "", $fila['precio']);
     ...
     //foreach ($array as $fila){
    if ($update_co && $insertar_fact_dett)
    {
      _commit(); // transaction is committed
      $xdatos['typeinfo']='Success';
      $xdatos['msg']='Cotización editada correctamente !';
    ...
    else
    {
      ...
      $xdatos['msg']='Cotización no pudo ser editada!'.$insertar_fact."-".$insertar_numdoc."-".$insertar_fact_dett._error();
    }
  }
