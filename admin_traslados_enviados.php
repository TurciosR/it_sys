<?php
	include ("_core.php");
	// Page setup
	$title= 'Administrar Traslados Enviados';
	$_PAGE = array ();
	$_PAGE ['title'] =$title ;
	$_PAGE ['links'] = null;
	...
	$sql="SELECT traslado.*, usuario.nombre as nombreuser,sucursal.descripcion FROM traslado JOIN usuario ON traslado.id_empleado=usuario.id_usuario JOIN sucursal ON traslado.id_sucursal_destino=sucursal.id_sucursal WHERE traslado.id_sucursal_origen='$id_sucursal'";
	$result=_query($sql);
				...
				<?php
 					if ($count>0){
						for($i=0;$i<$count;$i++){
							...
							$link=permission_usr($id_user,$filename);
							if ($link!='NOT' || $admin=='1' )
								echo "<li><a href='$filename?id_traslado=".$row ['idtransace']."&id_sucursal=$id_sucursal' target='_blank' ><i class=\"fa fa-search\"></i> Ver Detalle</a></li>";
							...
							if($txt_estado=='PENDIENTE' || $finalizada==0){
								...
								*/
								...
								if ($anulada==0&&$verificado==0) {
									# code...
									$link=permission_usr($id_user,$filename);
									if ($link!='NOT' || $admin=='1'){
										echo "<li><a data-toggle='modal' href='anular_traslado.php?id_traslado=" .  $row ['idtransace']."&process=formDelete"."' data-target='#deleteModal' data-refresh='true'><i class=\"fa fa-eraser\"></i> Anular</a></li>";
									}
								}
								/*$filename='editar_traslados.php';
								if ($verificado==0) {
									# code...
									$link=permission_usr($id_user,$filename);
									if ($link!='NOT' || $admin=='1'){
										echo "<li><a href='editar_traslados.php?id_traslado=" .  $row ['idtransace'].""."'><i class=\"fa fa-pencil\"></i> Editar Traslado</a></li>";
									}
								}*/
