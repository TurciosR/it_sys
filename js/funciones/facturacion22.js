...
function cargar_precios(){
	$('#inventable>tbody>tr').find("#select_precios").each(function(){
			$(this).load('facturacion.php?'+'process=cargar_precios');	
			totales();
		});
}
*/	
...
//var filas=0; 
	...
	if(unidades<1){
	 ...
	 var cprom0=parseFloat(cprom/unidades);
	 var pr1=round(parseFloat(cprom0)+parseFloat(cprom0*(p1/100)),2);
	 var pr2=round(parseFloat(cprom0)+parseFloat(cprom0*(p2/100)),2);
	 var pr3=round(parseFloat(cprom0)+parseFloat(cprom0*(p3/100)),2);
	 var pr4=round(parseFloat(cprom0)+parseFloat(cprom0*(p4/100)),2);	
	 
	 if (tipo=='SERVICIO'){
		pr1=pv;pr2=pv;pr3=pv;pr4=pv;
	 } 				
	select_precios="<select name='select_precios' id='select_precios' class='form-control'>";
	select_precios+="<option value="+pr1+">"+pr1+"</option>";
	select_precios+="<option value="+pr2+">"+pr2+"</option>";
	select_precios+="<option value="+pr3+">"+pr3+"</option>";
	select_precios+="<option value="+pr4+">"+pr4+"</option>";
