<?php
    ...
    $joinQuery ="FROM traslado JOIN empleados ON traslado.id_empleado=empleados.id_empleado JOIN sucursal ON traslado.id_sucursal_destino=sucursal.id_sucursal ";


    $extraWhere = "traslado.id_sucursal_origen='$id_sucursal'
    ...
    array( 'db' => '`traslado`.`idtransace`', 'dt' => 6, 'formatter' => function ($id_pedido, $row) {
        ...
        'field' => 'idtransace'),
        array( 'db' => '`traslado`.`idtransace`', 'dt' => 7, 'formatter' => function ($id_pedido, $row) {
            ...
            if ($anulada!=1) {
              ...
              if ($link!='NOT' || $admin=='1') {
                  $menudrop.= "<li><a href='$filename?id_traslado=$row[idtransace]&id_sucursal=$id_sucursal' target='_blank' ><i class='fa fa-print'></i> Ver Ficha Traslado</a></li>";
              }
            }
            ...
            if ($anulada==0&&$verificado==0) {
                ...
                $link=permission_usr($id_user, $filename);
                if ($link!='NOT' || $admin=='1') {
                    $menudrop.= "<li><a data-toggle='modal' href='anular_traslado.php?id_traslado=" .  $row ['idtransace']."&process=formDelete"."' data-target='#deleteModal' data-refresh='true'><i class='fa fa-eraser'></i> Anular</a></li>";
                }
            }
