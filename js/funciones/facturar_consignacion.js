...
$(document).on("change","#id_cliente", function (){
	...
	$('#datos_cliente').load('consignacion.php?'+'process=mostrar_datos_cliente'+'&id='+id); 
});		
*/
...
$(document).on("change","#tipo_entrada", function(){
	var id=$("select#tipo_entrada option:selected").val(); //get the value
	//alert(id);
	$('#mostrar_numero_doc').load('consignacion.php?'+'process=mostrar_numfact'+'&id='+id); 
});	
*/
/*
function cargar_empleados(){
	$('#inventable>tbody>tr').find("#select_empleado").each(function(){
			$(this).load('consignacion.php?'+'process=cargar_empleados');	
			totales();
		});
}
* */
...
$(document).on("keyup","#cant, #precio_venta, #cant_fact, #cant_facturado_ante",function(){
  totales();
})
...
function totales(){
	...
	var total_fact=0; cant_facturado_ante=0; cant_fact=0;subtot_consigna=0;total_cant_facturado_ante=0;subtotal_fact=0;
	...
	//$("#inventable>tbody tr").each(function (index) {
		...
		if (index>0){		
           var campo0,campo1, campo2, campo3, campo4, campo5, cant_consigna,campo6 ;
            $(this).children("td").each(function (index2) {
                switch (index2){
                    case 0: 
						campo0 = $(this).text();
                   ...
                   case 6:						
						...
						if (isNaN(campo6)==false)
							cant_facturado_ante=parseFloat(campo6);
						else
						...
						if(cant_fact>cant_max_facturar){
							...
							//$(this).find("#cant_facturado_ante").html("0");
            ...
            if(cant_fact>=cant_max_facturar){
				...
				/*
				cant_facturado_ante=cant_consigna-cant_fact;
				if (isNaN(cant_facturado_ante)==true)
					cant_facturado_ante=cant_consigna;		
				cant_facturado_ante_fin=cant_facturado_ante.toFixed(2);
				$(this).find("#cant_facturado_ante").html(cant_facturado_ante_fin);
				*/
			...
			total_cant_facturado_ante=parseFloat(total_cant_consigna) - parseFloat(total_cant_fact);            		  
          } 
        });
       
    total_dinero=total_dinero_fact.toFixed(2);
    total_cant_facturado_ante_fin=total_cant_facturado_ante.toFixed(2);
    $('#total_cant_fact').html(total_cant_fact);
    //$('#total_cant_facturado_ante').html(total_cant_facturado_ante_fin);
	$('#total_dinero_fact').html("<strong>"+total_dinero+"</strong>");	
	$('#totaltexto').load('consignacion.php?'+'process=total_texto&total='+total_dinero);
}	
...
function senddata(){
	...
	*/
	...
	//valores totales a enviar para validar consignacion y facturar
...
function imprimir_pdf(){
	...
	*/
	...
	cadena = "ver_reporte_consignacion.php?"+dataString ;	
	window.open(cadena, '', '');	    
}
