...
function totales(){
		...
		if (index>0){		
           var campo0,campo1, campo2, campo3, campo4, campo5, cant_consigna,campo6 ;
            ...
            if(cant_fact>=cant_consigna){
				...
				dev_stock=0;
				//cant_dev=0;
				subtotal_fact=precio_venta*cant_consigna;
				$(this).find("#subtot_fact").html(subtotal_fact);
			}
			else{
				dev_stock=cant_consigna-cant_fact;
				if (isNaN(dev_stock)==true)
					dev_stock=cant_consigna;		
			...
			total_dev_stock=parseFloat(total_cant_consigna) - parseFloat(total_cant_fact);            		  
          } 
        });
	...
	$('#totaltexto').load('finalizar_garantia.php?'+'process=total_texto&total='+total_dinero);
}	
...
function imprime2(){
 ...
 function reload1(){
	location.href = 'admin_garantias_nofin.php';	
}
function deleted() {
...
//Generar pdf de finalizar_garantia
function imprimir_pdf(){
	
	var numero_doc=$("#numero_doc").val();
	var dataString='numero_doc='+numero_doc; 
	cadena = "ver_reporte_finalizar_garantia.php?"+dataString ;	
	window.open(cadena, '', '');	    
}
