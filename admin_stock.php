...
function initial()
{
                          ...
                          <table class="table table-bordered" id='loadtable'>
                            <thead>
                              <tr>
                                ...
                                <th class="text-success col-lg-6">Producto</th>
                                <th class="text-success col-lg-3">Categoria</th>
  ...
  function traerdatos()
  {
    ...
    $sqlJoined="  SELECT pr.id_producto,pr.descripcion, pr.ultcosto, pr.barcode, c.nombre as cat, SUM(su.cantidad) as cantidad
    FROM productos AS pr, categoria AS c, stock_ubicacion AS su ";
    ...
    function traerpaginador()
    {
      ...
      if(isset($_POST['page']))
      {
        include('Pagination.php');
        $sqlParcial=get_sql($start,$limit,$keywords,$sortBy,$id_ubicacion,$barcode);
        $sql1="SELECT COUNT(DISTINCT(pr.id_producto)) as numRecords  FROM productos AS pr, categoria AS c, stock_ubicacion as su ";
        if($id_ubicacion !="")
        {
          $sql1="SELECT COUNT(DISTINCT(pr.id_producto)) as numRecords  FROM productos AS pr, categoria AS c, stock_ubicacion AS su ";
        }
