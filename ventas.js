...
$(document).on('change', ".precios", function()
{
  ...
  if(cantidad != "")
  {
    ...
    var cantidad = $(this).parents("tr").find('#sub_to').val(sub);
  }
  else
  {
...
$(document).on('keyup', "#cant", function()
{
  ...
  var precio = parseFloat($(this).parents('tr').find('.precios').val());

  if(valor > cantidad)
  {
    $(this).val("1");
    $(this).parents("tr").find('#sub_to').val(precio);
  }
  else
  {
    if(valor != "")
...
function agregar_servicio(id_ser) {
  ...
  var id_pedido = $("#select_pedidos").val();
  var dataString = 'process=consultar_servicio&id_servicio=' + id_ser;
