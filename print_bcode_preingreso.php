...
function initial() {
  ...
  $fecha=date('d-m-Y');
	...
	$sql0="SELECT monto, idtransace,pares,items
	FROM pedidos AS cp
...
<div class="modal-header">
	...
	<h4 class="modal-title">Ver Preingresos e Imprimir Barcodes</h4>
</div>

<div class="modal-body">
		<div class="row">
	<div class="col-md-16">&nbsp;&nbsp;
	<h3 class="text-navy" id='title-table'>&nbsp;Lista  del Preingreso  No. <?php echo $numero_doc;?></h3>
	</div>
		...
		<input type="text"  class='form-control input_header_panel'  id="pares"  value='<?php echo $pares;?>' readOnly /></label>
				...
				<?php
					...
					<div class="row" id="row1">
                     ...
                     <th class="text-success col11 th1">Precio</th>
										</tr>
									</thead>
...
function buscarcompras(){
	...
	pr.descripcion,pr.talla,pr.estilo,c.nombre,dc.ultcosto
	FROM pedidos AS cp
...
function buscarprodcant(){
	...
	pr.descripcion,pr.talla,c.nombre,dc.ultcosto,pr.id_proveedor
  FROM pedidos AS cp
	JOIN detalle_pedidos AS dc ON(cp.idtransace=dc.idtransace)
	JOIN productos AS pr ON(pr.id_producto=dc.id_producto)
	JOIN colores AS c ON (pr.id_color=c.id_color)
