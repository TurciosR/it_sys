...
function initial(){
	...
	//$sql="SELECT * FROM traslado WHERE id_traslado='$id_traslado'";
	$sql="SELECT traslado.*, cliente.nombre,cliente.apellido FROM traslado JOIN cliente 
	ON traslado.id_cliente=cliente.id_cliente
	WHERE id_traslado='$id_traslado' 
	AND traslado.id_sucursal='$id_sucursal'";
	$result = _query( $sql );
...
<div class="modal-header">
	...
	<h4 class="modal-title">Eliminar traslado</h4>
</div>
...
function deleted() {
	...
	$sql_fact="SELECT * FROM traslado WHERE id_traslado='$id_traslado' AND id_sucursal='$id_sucursal'";
	...
	//falta hacer update reingresar a stock si hay producto en traslado anulada	!!!!!!
	$sql_fact_det="SELECT  producto.id_producto, producto.descripcion,traslado_detalle.* 
		...
		AND  traslado_detalle.tipo_prod_serv='PRODUCTO' AND traslado_detalle.id_sucursal='$id_sucursal'";
	
	$result_fact_det=_query($sql_fact_det);
	$nrows_fact_det=_num_rows($result_fact_det);	   
	$tipo_entrada_salida='INGRESO POR BORRAR TRASLADO';
	for($i=0;$i<$nrows_fact_det;$i++){
		...
		if($nrow1>0){			
			...
			//Eliminar traslado
			$table = 'traslado';
			$where_clause_c = "id_traslado='".$id_traslado ."' AND id_sucursal='".$id_sucursal."'";
			$delete = _delete($table,$where_clause_c ); 
			$table_cd = 'traslado_detalle';
			$where_clause_cd = "id_traslado='".$id_traslado ."' AND id_sucursal='".$id_sucursal."'";
	...
	if ($delete) {
		...
		$xdatos ['msg'] = 'Traslado Eliminado!';
	} else {
		$xdatos ['typeinfo'] = 'Error';
		$xdatos ['msg'] = 'Traslado no Eliminado! ';
	}
