		...
		<div class="row">
          ...
          <div class="col-lg-12">
            ...
            <div  class='widget-content' id="content">
                          ...
                          <tfoot class='tbody1 ' id="mostrardatos2">
													</tfoot>
												</table>
										</div>
									</div>
...
function traerdatos(){
    ...
    $sqlJoined=" SELECT pr.id_producto,pr.barcode, pr.descripcion, pr.precio1, pr.precio2, pr.precio3,
      ...
      pv.nombre,st.existencias, gp.fecha_ini,gp.fecha_fin, gp.serie,gp.id_garantia
      ";
       $sqlFrom=" FROM productos AS pr, proveedores AS pv, stock AS st, garantia_prov_det as  gp";
      $limitSQL= " LIMIT $start,$limit ";
     $sqlParcial= get_sql($start,$limit,$sortBy,$keywords, $barcode, $marca, $modelo,$serie,$id_proveedor);
     $sql_final=$sqlJoined.$sqlFrom.$sqlParcial.$limitSQL;
    $query = _query($sql_final);
		$num_rows = _num_rows($query);
   //sql para traerpaginador
   $sqlJoined2="SELECT COUNT(*) as numRecords " ;

      $sql_pag=$sqlJoined2.$sqlFrom.$sqlParcial;
      $queryNum = _query($sql_pag);
      $resultNum = _fetch_array($queryNum);
      $rowCount = $resultNum['numRecords'];
  		//$rowCount  = _num_rows($query2);
          echo _error();
		$filas=0;
    $fecha_actual=date('Y-m-d');
    if($num_rows > 0){
            ...
            <td class='col25 td1'><?php echo $descripcion." ".$marca." ".$modelo. " ".$serie;?></td>
            ...
            <td class='col8 td1'><?php echo $estado;?></td>
            <td class='col8 td1'><?php echo $serie;?></td>
            <td class='col15 td1'><?php echo $nombre;?></td>

            <?php
                $acciones=acciones($id_producto,$id_garantia);
                ?>
                <td class='col7 td1 td_success'><div clas='col-xs'><?php echo $acciones;?></div></td>
                <?php
                  $filas+=1;
              ?>
            </tr>
        ...
        echo "<tr><td class='col100 td1' id='paginatorr'>";
        echo "<div clas='marg_der2'>";
        paginador($start,$limit,$sortBy,$rowCount);
        echo"</div></td></tr>";
...
function  get_sql($start,$limit,$sortBy,$keywords, $barcode, $marca, $modelo,$serie,$id_proveedor){
  ...
  $orderBy=" ORDER BY pr.id_producto,pr.descripcion, pr.barcode,pr.marca,pr.modelo ";
	...
	$sql_final=$whereSQL.$andSQL.$orderBy;
  return $sql_final;
}

function paginador($start,$limit,$sortBy,$rowCount){
...
function acciones($id_producto,$id_garantia){
  $menudrop="<div class='btn-group col-xs'>
  ...
  $filename='archivo_garantia_prov.php';
  $link=permission_usr($id_user,$filename);
  if ($link!='NOT' || $admin=='1' ){
    $menudrop.="<li><a a data-toggle='modal' href='$filename?id_garantia=".$id_garantia."' data-target='#viewModal' data-refresh='true'><i class='fa fa-upload'></i> Doc. Garantia</a></li>";
  }
