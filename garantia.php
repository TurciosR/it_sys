           ...
           <div class="row">
                <div class="col-lg-12">
                    <div class="ibox ">
					 ...
					 <div class="row">
						 <div id='form_datos_proveedor'>
							 <input type='hidden' name='idfactura' id='idfactura' value=''>
                            <div class="col-lg-6">
                              <div class="form-group">
								  <label>Seleccione Proveddor</label><br>
							<?php
								echo"<select  name='id_proveedor' id='id_proveedor'  class='form-control' style='width:300px;'>";
                     ...
                     echo "</div>";//<!--<div id='form_datos_proveedor'> -->
...
echo "<script src='js/funciones/garantia.js'></script>";
} //permiso del script
else {
...
function insertar(){
	...
	$tipo_entrada_salida='garantia SALIDA';
	...
	$numero_docx=numero_tiquete($ult_gar);
	$numero_doc =$numero_docx."_GAR";
	...
	//if ($id=='1'){
		...
		if ($cuantos>0){
			//insertar a la tabla garantia
			//INSERT INTO garantia(id_garantia, id_proveedor, fecha, numero_doc, total, id_usuario) VALUES ();
			$sql_fact="SELECT * FROM garantia WHERE numero_doc='$numero_doc' and fecha='$fecha_movimiento' AND id_sucursal='$id_sucursal'";
			...
			for ($i=0;$i<$cuantos ;$i++){
				...
				$table_fact_det= 'garantia_detalle';
				$data_fact_det = array(
				'id_garantia' => $id_fact,
			...
			if($insertar1 && $insertar2 && $insertar_fact && $insertar_fact_det){
					...
					$xdatos['msg']='garantia Numero: <strong>'.$numero_doc.'</strong>  Guardada con Exito !';
  ...
  /*
    ...
    if(!$insertar1 && !$insertar2 && !$insertar_fact &&$insertar_fact_det){
       $xdatos['typeinfo']='Error';
       $xdatos['msg']='Registro de garantia no pudo ser Actualizado !';
		}
*/
...
function mostrar_datos_proveedor(){
	$id=$_REQUEST['id'];
...
function print1(){
	...
	$info_garantia.=$esp_init.$numfact."\n";
	for($j=0;$j<2;$j++){
		$info_garantia.="\n";
	}
	$info_garantia.=$esp_init2.$nombre_ape.$esp_enc2.$fecha_movimiento."\n";
	$info_garantia.=$esp_init2.$dir_txt.$esp_enc2.$dui."\n";
	for($j=0;$j<3;$j++){
		$info_garantia.="\n";
	}
	$listadatos=explode('#',$stringdatos);
	...
	$info_garantia.=$esp_init2.$salida_txt_total.$esp."$ ".$total_value."\n";
	$esp=espacios_izq(" ",86);
	$info_garantia.="\n";
	$info_garantia.=$esp_init.$esp." $ ".$total_value."\n";
	$nreg_encode['garantiar'] = $info_garantia;
	$nreg_encode['sist_ope'] =$so_proveedor;
...
function print2(){
	...
	$id = $_POST['id'];
	if ($id=='1'){
		$tipo_entrada_salida='garantia CONSUMIDOR';
		$fecha_movimiento= $_POST['fecha_movimiento'];
		$numero_doc = $_POST['numero_doc'];
	...
	if ($id=='2'){
		$tipo_entrada_salida='garantia CREDITO FISCAL';
		$id_proveedor=$_POST['id_proveedor'];
	...
	$info_garantia.=$esp_init2.$nombre_ape.$fecha_movimiento."\n";
	$info_garantia.=$esp_init2.$dir_txt.$dui."\n";
	for($j=0;$j<2;$j++){
			$info_garantia.="\n";
	}
	$listadatos=explode('#',$stringdatos);
	...
	$esp=espacios_izq(" ",86);
	$info_garantia.="\n";
	$info_garantia.=$esp_init.$esp." $ ".$total_final."\n";
	$nreg_encode['garantiar'] = $info_garantia;
	$nreg_encode['sist_ope'] =$so_proveedor;
	echo json_encode($nreg_encode);
...
function reimprimir() {
	...
	$info_garantia="";
	$sql_fact="SELECT * FROM garantia WHERE numero_doc='$numero_docx'";
	$result_fact=_query($sql_fact);
	$row_fact=_fetch_array($result_fact);
	$nrows_fact=_num_rows($result_fact);
	if($nrows_fact>0){
		...
		$info_garantia.=$esp_init2.$nombre_ape.$esp_enc2.$fecha."\n";
		$info_garantia.=$esp_init2.$dir_txt.$esp_enc2.$dui."\n";
		for($j=0;$j<3;$j++){
			$info_garantia.="\n";
		}
		...
		UNION ALL
		SELECT  servicio.id_servicio, servicio.descripcion,garantia_detalle.*
		FROM garantia_detalle JOIN servicio ON garantia_detalle.id_prod_serv=servicio.id_servicio
		WHERE  garantia_detalle.id_garantia='$id_garantia' AND  garantia_detalle.tipo_prod_serv='SERVICIO'
		";
...
//mostrar el num de garantia de consumidor o credito fiscal
function mostrar_numfact(){
	...
	*/
	echo "<div class='form-group has-info single-line'><label>Numero de garantia</label> <input type='text' placeholder='Numero de garantia' class='form-control' id='numero_doc' name='numero_doc' value='$valor_txt'></div>";

	//echo "<div class='col-md-4'>";
	//echo "<div class='form-group has-info single-line'><label>Numero de garantia</label> <input type='text' placeholder='Numero de garantia' class='form-control' id='numero_doc' name='numero_doc' value='$ult_doc'></div>";
	//echo "</div>";
	//echo "<div class='col-md-4'>";
	//echo "<div class='form-group has-info single-line'><label>Fecha:</label> <input type='text' placeholder='Fecha' class='datepick form-control' id='fecha' name='fecha' value='$fecha_actual'></div>";
	//echo "</div>";
...
if (isset($_REQUEST['process'])) {
	...
	case 'print1':
		...
		break;
	case 'print2':
		print2(); //Generacion de los datos de garantia que se retornan para otro script que imprime!!!
		break;
