            ...
            <div  class='widget-content' id="content">
											...
											<div class="col-md-12">
												...
												<table class="table table-striped" id='loadtable'>
													<thead class='thead1'>
														<tr class='tr1'>
														<th class="text-success col9 th1" >Código</th>
														<th class="text-success col19 th1">Nombre</th>
														<th class="text-success col9 th1">Estilo</th>
                            <th class="text-success col9 th1">Color</th>
														<th class="text-success col9 th1">Talla</th>
                            <th class="text-success col9 th1">Precio</th>
														<th class="text-success col9 th1">% Desc.</th>
                            <th class="red1 col9 th1">Exist. Centro </th>
                            <th class="blue1 col9 th1">Exist. Metro </th>
                            <th class="text-success col9 th1">Acci&oacute;n</th>
												</tr>
												</thead>
...
echo "<script src='js/funciones/cons_stock_sucursal.js'></script>";
}
...
";
  ...
  $groupBy=" GROUP BY pr.id_producto";
  $orderBy=" ORDER BY  pr.estilo,pr.talla ASC";
        ...
        while ($row = _fetch_array($query)) {
            ...
            $btnSelect='<input type="button" id="btnSelect" class="btn btn-primary fa" value="&#xf00c;">'; ?>
				   <tr class='tr1'  tabindex="<?php echo $filas; ?>">
					        <td class='col9 td1'><input type='hidden'  id='exento' name='exento' value='<?php echo $exento; ?>'> <h5><?php echo $id_producto; ?></h5></td>
                    <td class='col19 td1'><?php echo $descripcion; ?></td>
                    <td class='col9 td1 centrar'><?php echo $estilo; ?></td>
                    <td class='col9 td1'><?php echo $nombre; ?></td>
                    <td class='col9 td1'><?php echo $talla; ?></td>
                    <td class='col9 td1 td_success'>$ <?php echo $precio; ?></td>
					          <td class='col9 td1 centrar'><?php echo $descuento; ?></td>
                    <td class='col9 td1 red1 centrar'><?php echo $sucursal_1; ?></td>
                    <td class='col9 td1 blue1 centrar'><?php echo $sucursal_2; ?></td>

          <?php
            $acciones=acciones($id_producto);
            ?>
            <td class='col9 td1 td_success'><?php echo $acciones;?></td>
            <?php
              $filas+=1;
        }
    }
...
function acciones($id_producto){
  ...
  $link=permission_usr($id_user,$filename);
  if ($link!='NOT' || $admin=='1' ){
    $menudrop.="<li><a href='$filename?id_producto=".$id_producto."&process=formDelete"."' data-target='#deleteModal' data-refresh='true'><i class='fa fa-eraser'></i> Eliminar</a></li>";
  }
  $filename='ver_producto.php';
  $link=permission_usr($id_user,$filename);
  if ($link!='NOT' || $admin=='1' ){
    $menudrop.="<li><a href='$filename?id_producto=".$id_producto."' data-target='#viewModal' data-refresh='true'><i class='fa fa-search'></i> Ver Detalle</a></li>";
  }
  $menudrop.="</ul>
        </div>";
  return $menudrop;
