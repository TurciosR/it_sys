...
function initial() {
	$title="Venta Continua sin Impresión";
	$_PAGE = array ();
	$_PAGE ['title'] = $title;
	$_PAGE ['links'] = null;
           ...
           <div class="row">
					 ...
					 <div class="row">
						 <div id='form_datos_cliente'>
							 <div class="form-group col-md-4">
								 <div class="form-group"><label>Seleccione cliente&nbsp;</label></div>
								 <?php
			...
			<div class="col-md-3">
				<div class="form-group has-info">
					...
					<select  name='tipo_impresion' id='tipo_impresion'  class='form-control' disabled>
					<option value='FLC' selected>FACTURACION POR LOTE</option>
							...
							<section>
								...
								<div style="margin:7px">
									...
									<div class='form-group has-info single-line'><label id='buscar_habilitado'>Buscar Producto o Servicio</label>
										...
										<input  type="text" id="producto_buscar2" name="producto_buscar2" size="20" class="form-control" placeholder="Ingrese  barcode producto o servicio facturar"  style="border-radius:0px" >
									</div>
									</div>
									</div>
		...
		<div class="modal-dialog">
			<div class="modal-content modal-sm">
				...
				<div class="modal-footer">
					...
					<button type="button"  class="btn btn-success" id="btnFinFact" ><i class="fa fa-check"></i> Guardar</button>
				</div>
				</div>
			</div>
		</div>
...
echo "<script src='js/funciones/genera_venta_lote.js'></script>";
} //permiso del script
else {
	...
	if($cuantos>0 && $stringdatos!="" && $total_venta>0 ){
		...
		if ($cuantos>0){
			...
			if($insertar_numdoc  && $insertar2  && $insertar_fact && $insertar_fact_det && $insertar_mov_prod){
					...
					$xdatos['insertados']=" num_doc :".$numero_doc." factura :".$insertar_fact." factura detalle:".$insertar_fact_det." mov prod:".$insertar1." stock:".$insertar_mov_prod." lote:".$insertar4  ;
					}
			}//if
...
function imprimir_fact() {
	...
	$id_ccf_cof= $_POST['tipo_impresion'];
 $tipo_impresion= $_POST['tipo_impresion'];
	$id_sucursal=$_SESSION['id_sucursal'];
	//$id_ccf_cof='1';
	//$tipo_fact="idfact";
...
if ($tipo_impresion=='TIK'){
	$info_facturas=print_ticket($id_factura,$tipo_fact);
}
	//$tipo_fact="idfact";
