...
function initial() 
{
    ...
    $sql = _query("SELECT * FROM bancos WHERE id_banco='$id_banco'");
...
function insertar()
{
    ...
    $sql_exis=_query("SELECT id_banco FROM bancos WHERE nombre='$nombre' AND id_banco != '$id_banco'");
    $num_exis = _num_rows($sql_exis);
    if($num_exis > 0)
    {
        $xdatos['typeinfo']='Error';
        $xdatos['msg']='Este banco ya fue registrado!';
    }
    else
    {
        if ($_FILES["logo"]["name"]!="")
        { 
            ...
            if ($foo->uploaded)
            {
               ...
               if ($foo->processed) 
               {
                    $query = _query("SELECT logo FROM bancos WHERE id_banco='$id_banco'");
