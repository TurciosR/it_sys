...
<?php
	...
	<table align='center'  class='headers_perpage' cellspacing='1' style='width: 92.6%; border: solid 1px black; background: #A9A9A9; text-align: center; font-size: 11pt; color:#FFFFFF; font-family:helvetica,serif;'>
        <tr>
            ...
            <th style='width:20%;text-align:left';>Descripcion Producto</th>
            <th style='width:10%;text-align:left';>Factura No.</th>
...
<?php
	/*
	$total_productos="SELECT id_prod_serv as id_prod,descripcion, empleado.id_empleado,empleado.nombre,empleado.apellido,
	factura_detalle.cantidad as cant_prod_empl,factura_detalle.precio_venta as precio_venta,
	factura_detalle.subtotal as subtotal_empleado,factura.numero_doc
	FROM factura,factura_detalle,producto, empleado 
	WHERE factura.id_factura=factura_detalle.id_factura AND id_prod_serv=id_producto 
	AND empleado.id_empleado=factura_detalle.id_empleado 
	AND factura_detalle.tipo_prod_serv='PRODUCTO' AND factura.anulada=0 AND DATE( factura.fecha )
	BETWEEN '$fecha_inicio' AND '$fecha_fin'
	AND factura_detalle.precio_venta>0
	UNION ALL
	SELECT id_prod_serv as id_prod,descripcion, empleado.id_empleado,empleado.nombre,empleado.apellido,
	factura_detalle.cantidad as cant_prod_empl,factura_detalle.precio_venta as precio_venta,
	factura_detalle.subtotal as subtotal_empleado,factura.numero_doc
	FROM factura,factura_detalle,producto, empleado 
	WHERE factura.id_factura=factura_detalle.id_factura AND id_prod_serv=id_producto 
	...
	BETWEEN '$fecha_inicio' AND '$fecha_fin'
	AND factura_detalle.precio_venta<=0
	ORDER BY id_empleado,precio_venta";
	*/
	$total_productos="SELECT id_prod_serv as id_prod,descripcion, usuario.id_usuario,usuario.nombre,
	factura_detalle.cantidad as cant_prod_empl,factura_detalle.precio_venta as precio_venta,
	factura_detalle.subtotal as subtotal_empleado,factura.numero_doc
	FROM factura,factura_detalle,producto, empleado 
	WHERE factura.id_factura=factura_detalle.id_factura 
	AND id_prod_serv=id_producto 
	AND usuario.id_usuario=factura_detalle.id_empleado 
	AND factura_detalle.tipo_prod_serv='PRODUCTO' AND factura.anulada=0 AND DATE( factura.fecha )
	BETWEEN '$fecha_inicio' AND '$fecha_fin'
	AND factura_detalle.precio_venta>0
    ...
    $numero_doc = $total_serv_empleado['numero_doc'];
    list($numfact,$tipofact)=explode("_",$numero_doc);
    ...
    <table border='0.5px' align='center' cellspacing='0'  style='width: 100%; border:none; text-align: center; font-size: 11pt; color:#000; font-family:helvetica,serif;'>
		...
		echo "<td style='width:20%;text-align:left'>".$nombre." ".$apellido."</td>";
	    echo "<td style='width:20%;text-align:left'>".$descripcion."</td>";        
	    echo "<td style='width:10%;text-align:left'>".$numfact."</td>"; 
        echo "<td style='width:10%;text-align:right'>".$cant_prod_empl."</td>";
		if($precio_venta>0)
			echo "<td style='width:15%;text-align:right'>".$precio_venta."</td>";
        else
			echo "<td style='width:15%;text-align:center'>"." GRATIS "."</td>";
...
ROUND(SUM(factura_detalle.cantidad), 2) as cant_prod_empleado, 
...
union all
SELECT empleado.id_empleado,empleado.nombre,empleado.apellido,
ROUND(SUM(factura_detalle.cantidad), 2) as cant_prod_empleado, 
ROUND(SUM(factura_detalle.subtotal), 2) as total_empleado 
FROM factura,factura_detalle,producto, empleado
WHERE factura.id_factura=factura_detalle.id_factura
...
";
*/
$total_empleados= "SELECT  usuario.id_usuario,usuario.nombre,
ROUND(SUM(factura_detalle.cantidad), 2) as cant_prod_empleado, 
ROUND(SUM(factura_detalle.subtotal), 2) as total_empleado 
FROM factura,factura_detalle,producto, empleado
WHERE factura.id_factura=factura_detalle.id_factura
...
";
 //inicia validar consolidado
    ...
    $cant_producto =$row_total_empleado['cant_prod_empleado'];
    ...
    echo " <table border='0.5px' align='center' cellspacing='0'  style='width: 100%; border:none; text-align: center; font-size: 11pt; color:#000; font-family:helvetica,serif;'>
		...
		if($total_empleado>0)
			...
			echo "<td style='width:30%;text-align:center'>"." GRATIS "."</td>";
