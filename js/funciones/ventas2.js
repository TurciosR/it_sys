...
$(document).ready(function()
{
  ...
  $("#example-table").tabulator({
    ...
    columns:[
      ...
      <th class="text-success col8 th1">Subtotal</th>
      <th class="text-success col8 th1">$ Descto</th>
      <th class="text-success col8 th1">Total</th>
      <th class="text-success col8 th1">Garantia </th>
      <th class="text-success col8 th1">Acci&oacute;n</th
      */
        {title:"Id", field:"id", width:20, editable:false},
        {title:"Nombre", field:"descrip", width:200, editable:true},
        {title:"Existencias", field:"exist", width:100, align:"right", sorter:"number", editable:false},
        {title:"Precio L.", field:"prlist",editor:"select",editorParams:{"male":"Male", "female":"Female"}, editable:false},
        {title:"Precio", field:"precio", align:"center", width:80, editable:true},
        {title:"cantidad", field:"cant", editable:true},
        {title:"Subtotal", field:"subtotal", align:"center",  editable:false},
        {title:"$ Descto", field:"descto", align:"center", editable:true},
          {title:"total", field:"total", align:"center",  editable:false},
            {title:"Garantia", field:"garantia", align:"center",  editable:false},
          {title:"Borrar", field:"delete", align:"center",  editable:true},
    ],
});
...
function addProductList(id_prod,cantidad) {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      // var subtotal=parseFloat(precios_vta)*cantidad;
       ...
       if (exento==1){
        ...
        pr1=precios_vta[0];
  		  var subtotal=subt(1,data.precio1);
    ...
    //  if (existencias>0){
     tr_add = '';
     tr_add += '<tr class="tr1">';
     var sel_precio=data.select_precios
     tr_add += "<td class='col5 td1'><input type='hidden'  id='exento' name='exento' value='" + exento + "'>" + id_prod + "</td>";
     tr_add += '<td  class="col20 td1">' + descrip +' '+marca+' modelo:'+modelo+ '</td>';
     tr_add += "<td class='text-success col5 td1' id='cant_stock'>" + existencias + "</td>";
     tr_add += "<td class='text-success col10 td1' id='select_prec'>" + data.select_precios + "</td>";
     tr_add += "<td class='text-success col10 td1'><input type='hidden'  id='precio_venta_inicial' name='precio_venta_inicial' value='"+data.precio1+"'><div class='col-xs-2'><input type='text'  class='form-control decimal'  id='precio_venta' name='precio_venta' value='"+data.precio1+"' style='width:60px;'></div></td>";
     tr_add += "<td class='text-success col10 td1'><input type='hidden'  id='cant_restada' name='cant_restada' value='"+"1"+"'><div class='col-xs-2'><input type='text'  class='form-control decimal qty' id='cant' name='cant' value='"+cantidad+"' style='width:60px;'></div></td>";
     tr_add += "<td class='text-success col8 td1 subtotals' id='subtotal'>"+subt_mostrar+"</td>";
     tr_add += "<td class='col8 td1'><div class='col-xs-2'><input type='text'  class='form-control decimal' id='descto' name='descto'  value='"+valor_descuento+"' style='width:60px;'></div></td>";
     tr_add += '<td class="col8 td1">'+"<input type='hidden'  id='subt_descto' name='subt_descto' value='0.0'>"  + subt_gravado+ '</td>';
     ...
     var tipo_impresion = $("#tipo_impresion  option:selected").val();
           ...
           if (pass===false) {
             ...
             if (existe == 0 && existencias>0)
             {
               ...
               $(".sel").select2();
               $(".decimal").numeric();
                $("#example-table").tabulator("addRow", {id:id_prod,descrip:descrip,exist:1,prlist:sel_precio,precio:1,cant:1,subtotal:1,descto:1,total:1,garantia:1,delete:1});
             }
      }
      totales();
      ads = 0;
...
function setRowCant(rowId) {
  ...
  var stock = parseFloat(tr.find('td:eq(2)').text());
  ...
  tr.find('td:eq(6)').text(subt_mostrar);
  ...
  tr.find('td:eq(8)').text(subt_mostrar2);
  totales();
};
...
$(document).on('change', '.sel_prec', function(){
  ...
  tr.find("#precio_venta").val(precio_sel);
...
$(document).on('keyup', '#descto', function(){
  var tr = $(this).parents("tr");
  var subtotal1 = tr.find('td:eq(6)').text();
  var descuento = $(this).val();

  if (parseFloat(descuento)>parseFloat(subtotal1 )){
    descuento=subtotal1
    tr.find('td:eq(7)').find("#descto").val(subtotal1);
  }
  if (isNaN(descuento)|| descuento==""){
    descuento=0;
  }
  var subtotal=parseFloat(subtotal1)-parseFloat(descuento);

  var subt_mostrar=subtotal.toFixed(2);
  tr.find('td:eq(8)').text(subt_mostrar);
  //tr.find('td:eq(9)').text(subt_mostrar);
  totales();
});
