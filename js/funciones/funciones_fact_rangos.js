...
function deleted() {
	var id_transace = $('#id_factura').val();
...
function imprime1(){
  ...
  var nit = $("#nit").val(); //del modal
  var nrc = $("#nrc").val(); //del modal
  var direccion = $("#direccion").val(); //del modal
  var nombre_clie = $("#nombre_clie").val(); //del modal
  var print = 'imprimir_fact';

	var dataString = "process=imprimir_fact&id_factura="+id_transace+"&tipo_impresion="+tipo_impresion;
	dataString += "&nombre_cliente="+nombre_clie+"&direccion="+direccion+"&nit="+nit+"&nrc="+nrc;
  $.ajax({
    type: 'POST',
    url: "ver_ventas.php",
    data: dataString,
