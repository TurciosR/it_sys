				...
				<?php
 					if ($count>0){
						for($i=0;$i<$count;$i++){
							...
							$fecha_entrega=ed($row['fecha_entrega']);
							...
							echo "<td>".$lugar_entrega."</td>";
							echo "<td><h5 class='text-success'>"."$ ".$total_consignado_format."</h5></td>";
							/*
							echo "<td>";
							...
							*/
							...
							$filename='finalizar_pedido.php';
							$link=permission_usr($id_user,$filename);
							if ($link!='NOT' || $admin=='1' ){
								echo"<li><a  href='$filename?id_pedido=" .  $row ['id_pedido']."'><i class='fa fa-check'></i> Finalizar</a></li>";
							}
							...
							//Pedido a Consigna
							$filename='pedido_para_consigna.php';
							$link=permission_usr($id_user,$filename);
							if ($link!='NOT' || $admin=='1' ){
								if ($finalizada==0 && $anulada==0){
									echo"<li><a data-toggle='modal' href='$filename?id_pedido=" .  $row ['id_pedido']."&process=formAnular"."' data-target='#deleteModal' data-refresh='true'><i class=\"fa fa-plus\"></i>Convertir a Consigna</a></li>";
								}
							}
							/*
							$filename='ver_facturas_pedido.php';
							$link=permission_usr($id_user,$filename);
							if ($link!='NOT' || $admin=='1' )
								echo"<li><a data-toggle='modal' href='$filename?id_pedido=" .  $row ['id_pedido']."' data-target='#viewModalFact' data-refresh='true'><i class=\"fa fa-plus\"></i> Ver Facturas pedido</a></li>";
								*/
							echo "	</ul>
										</div>
										</td>
