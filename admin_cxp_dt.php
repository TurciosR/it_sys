<?php
  ...
  /*
    ...
    */
	...
	";
    $extraWhere = "cxp.id_sucursal='$id_sucursal' AND pro.id_proveedor=$id_proveedor
	AND cxp.fecha BETWEEN '$fechai' AND '$fechaf'";
    //$extraWhere = "";
    $columns = array(
    array( 'db' => '`cxp`.`idtransace`', 'dt' => 0, 'field' => 'idtransace' ),
    array( 'db' => '`cxp`.`fecha`', 'dt' =>1, 'field' => 'fecha' ),
    array( 'db' => '`cxp`.`alias_tipodoc`', 'dt' => 2, 'field' => 'alias_tipodoc' ),
    array( 'db' => '`cxp`.`numero_doc`', 'dt' => 3, 'field' => 'numero_doc' ),
    array( 'db' => '`cxp`.`monto`', 'dt' =>4, 'field' => 'monto' ),
    array( 'db' => '`cxp`.`saldo_pend`', 'dt' =>5, 'field' => 'saldo_pend' ),
    array( 'db' => '`cxp`.`fecha_vence`', 'dt' =>6, 'field' => 'fecha_vence' ),
    array( 'db' => '`cxp`.`idtransace`', 'dt' => 7, 'formatter' => function ($idtransace, $row) {
...
{
    $id_sucursal=$_SESSION["id_sucursal"];
    $sql="SELECT saldo_pend FROM cxp WHERE idtransace='$idtransace'";
    $result=_query($sql);
    $count=_num_rows($result);
    $row=_fetch_array($result);
