...
class PDF_HTML_Table extends FPDF
{
    ...
    public function Footer()
    {
        ...
        $this->Cell(0, 0.4, 'Pag. '.$this->PageNo().' de {nb}', 0, 0, 'R');
    }


    public function PDF($orientation='L', $unit='mm', $format='Letter')
    {
    ...
    public function WriteHTML2($html)
    {
        //HTML parser
    $html=str_replace("\n", ' ', $html);
    ...
    public function WriteTable($data, $w)
    {
        //$this->HeaderTable();
        $this->SetLineWidth(.3);
        ...
        $this->SetTextColor(0);
        $this->SetFont('');
        //Escribimos primero el encabezado la primera vez

        //se escriben las filas de la tabla
        foreach ($data as $row) {
            $nb=0;
    ...
    if ($this->GetY()+$h>$this->PageBreakTrigger|| $this->GetY()>185.5) {
        ...
        if ($start!==false && $end!==false) {
            ...
            for ($i=1;$i<=count($tableData[0]);$i++) {
                if ($this->CurOrientation=='L') {
                    $w[] = abs(120/(count($tableData[0])-1))+12;
                } else {
