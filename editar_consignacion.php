...
?>
    ...
    <div class="row wrapper border-bottom white-bg page-heading">                
        ...
        <div class="wrapper wrapper-content  animated fadeInRight">
           <div class="row">
                <div class="col-lg-12">
					...
					<?php
		if ($finalizada==0){			
	     //Datos de Consignacion y consignacion_detalle
	     
		$sql_fact_det="SELECT  producto.id_producto,stock.stock, producto.descripcion,consignacion_detalle.* 
		FROM consignacion_detalle 
		JOIN producto ON consignacion_detalle.id_prod_serv=producto.id_producto
		JOIN stock ON consignacion_detalle.id_prod_serv=stock.id_producto
	    ...
	    AND consignacion_detalle.id_sucursal=stock.id_sucursal 
		";
...
echo "<script src='js/funciones/editar_consignacion.js'></script>";
...
function insertar(){
	...
	$tmp_tbl='CREATE TEMPORARY TABLE IF NOT EXISTS tmp1  LIKE consignacion_detalle';
	...
	if($nrows>0){
		...
		for ($i=0;$i<$cuantos ;$i++){
			...
			$sql1="SELECT * FROM consignacion_detalle
			...
			if($nrow_cat>0 && $nrows1>0){
				$where_clause=" WHERE id_prod_serv='$id'  AND id_consignacion='$id_consignacion'";
		...
		//Delete the tables that donot exist any more int the `otherTbl`
			$where_clause1=" WHERE consignacion_detalle.id_consignacion='$id_consignacion' AND consignacion_detalle.id_prod_serv  NOT IN (SELECT id_prod_serv FROM tmp1)";	
			$delete1 = _delete( $table2 ,$where_clause1); 
			$drop1=" DROP TABLE tmp1";		
