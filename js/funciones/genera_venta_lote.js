...
//busqueda con el plugin autocomplete typeahead
  ...
  // Tecla de funcion  F9 que guarda la factura....!
...
function senddata() {
  ...
  dataString+= '&total_ventas=' + total_ventas + '&id_empleado=' + id_empleado + '&tipo_impresion=' + tipo_impresion;
  if(i>0 && StringDatos!=""){
  $.ajax({
    type: 'POST',
    url: urlfactura,
...
function imprime1() {
  ...
  var dataString = 'process=' + print + '&numero_doc=' + numero_doc + '&tipo_impresion=' + tipo_impresion + '&num_doc_fact=' + num_doc_fact;
  ...
  $.ajax({
    ...
    success: function(datos) {
      ...
      if (tipo_impresion == 'COF') {
        if (sist_ope == 'win') {
          $.post("http://localhost/pueblo/printfact1.php", {
...
function reload1() {
  location.href = "genera_venta_lote.php";
}
