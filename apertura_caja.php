                                ...
                                <div class="form-group has-info single-line">
                                    <label>Empleado <span style="color:red;">*</span></label>
                                        <?php
                                            if($admin != 1)
                                            {
                                              $sql_empleado = _query("SELECT * FROM empleados WHERE id_empleado = '$id_user'");
                                              $cuen = _num_rows($sql_empleado);
                                              $row_empleado = _fetch_array($sql_empleado);

                                              $id_usuario = $row_empleado["id_empleado"];
                                            ...
                                            {
                                              $sql_empleado = _query("SELECT * FROM empleados WHERE tipo_empleado = 3 or admin = 1");
                                              $cuen = _num_rows($sql_empleado);
...
function apertura()
{
    ...
    /*$sqla=_query("SELECT DISTINCT id_producto FROM stock");

    $totala=0;
    while ($rowa=_fetch_array($sqla)) {

      // code...
      $sql2a=_query("SELECT id_presentacion FROM presentacion_producto WHERE id_producto=$rowa[id_producto] ORDER BY unidad ASC limit 1");

      while ($row2a=_fetch_array($sql2a)) {
        $sql3a=_fetch_array(_query("SELECT stock.stock*presentacion_producto.costo AS stock FROM stock JOIN presentacion_producto ON presentacion_producto.id_producto=stock.id_producto WHERE presentacion_producto.id_presentacion=$row2a[id_presentacion]"));
        $totala=$totala+$sql3a['stock'];
      }
    }*/
...
function apertura_turno()
{
    ...
    if($cuenta == 1)
    {
        ...
        {
            $ta = "apertura_caja";
            $ff = array('id_empleado' => $emp ,);
            $w = "id_apertura='".$id_apertura."'";

            $update_d = _update($ta, $ff, $w);
            $xdatos['typeinfo']='Success';
