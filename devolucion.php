...
function initial() {
	...
	$sqlc=_fetch_array(_query("SELECT CONCAT(clientes.nombre) as nombre FROM clientes WHERE id_cliente=$id_cliente"));
						...
						<div class="row">
							...
							<div class="col-lg-12">
								<table id="tabla" class=" table table-hover table-striped">
									...
									<?php
									$sql=_query("SELECT factura_detalle.id_factura_detalle, factura_detalle.id_prod_serv,productos.descripcion, factura_detalle.precio_venta,factura_detalle.cantidad FROM factura_detalle JOIN productos ON productos.id_producto=factura_detalle.id_prod_serv WHERE factura_detalle.id_factura='$id_factura'");
...
function devolucion()
{
  ...
  $form_data = array(
    ...
    $sql_costo = _query("SELECT ultcosto FROM productos WHERE id_producto=$id_producto");
    $datos_costo = _fetch_array($sql_costo);
    $precio_costo = $datos_costo["ultcosto"];
    ...
    if(!$insert_mov_det)
    {
      $j = 0;
		...
		$form_data2 = array(
			'id_producto' => $id_producto,
			'stock' => $cant_total,
			'costo_unitario'=>round(($precio_costo),2),
