    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        ...
        <div class="wrapper wrapper-content  animated fadeInRight">
           <div class="row">
                <div class="col-lg-12">
					...
					<header>
						<h4 class="text-navy">pedido</h4></header>
					...
					<?php
		if ($finalizada==0){
	     //Datos de Consignacin y pedido_detalle
		...
		?>
							...
							<tfoot>
									...
									<td id='total_dev_stock'><?php echo $total_dev_stock; ?></td>
...
echo "<script src='js/funciones/finalizar_pedido.js'></script>";
...
function insertar(){
	...
	$nrow=_num_rows($result_user);
	$id_sucursal=$_SESSION['id_sucursal'];
	//Traer datos encabezados de la pedido para insertarlos en factura
	$sql="SELECT *
		FROM pedido
		WHERE finalizada=0
	...
	if ($cuantos>0 ){
		...
		if($nrows_fact==0){
			//revisar insertar factura que traiga al menos 1 cantidad de productos sumando en el js cant fact no solo el num de lineas
			_begin();
		...
		for ($i=0;$i<$cuantos ;$i++){
			...
			if ($cantidad>0 && $total_cant_fact>0){
            ...
            $tipo_entrada_salida2='DEVOLUCION POR pedido No.'.$numero_doc;
