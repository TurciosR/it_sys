...
$(document).on("click", "#btnRemesa", function()
{
	document.getElementById("btnRemesa").disabled = true; 
	var fecha_fac = $("#fecha_fac").val();
	var cajero = $("#cajero").val();
	var turno_fac = $("#turno_fac").val();
	var n_remesa = $("#n_remesa").val();
	var remesa = $("#remesa").val();
	var banco = $("#bancoo").val();
	var cuenta = $("#cuentaa").val();
	var process = "remesa";
	var admin=$("#admin").val();
	if(n_remesa!=""){
	if(remesa!=""){
	...
	$.ajax({
		type:'POST',
		url:"agregar_remesa.php",
		data: "process="+process+"&fecha_fac="+fecha_fac+"&id_empleado="+cajero+'&turno_fac='+turno_fac+'&admin='+admin+'&n_remesa='+n_remesa+'&remesa='+remesa+'&banco='+banco+'&cuenta='+cuenta,
		dataType: 'json',
		success: function(datax){
			//var total=datax.total;
			//$('#total_sistema').val(total);
			...
			{
				display_notify(datax.typeinfo, datax.msg);
				document.getElementById("btnRemesa").disabled = false; 

			}
		}
	});
	}else{display_notify('Warning', 'Seleccione Cuenta!');
		document.getElementById("btnRemesa").disabled = false; 
	}
	}else{display_notify('Warning', 'Seleccione Banco!');
		document.getElementById("btnRemesa").disabled = false; 
	}
	}else{display_notify('Warning', 'El monto de Remesa es requerido!');
		document.getElementById("btnRemesa").disabled = false; 
	}
	}else{display_notify('Warning', 'El N° de Remesa es requerido!');
		document.getElementById("btnRemesa").disabled = false; 
	}
})
